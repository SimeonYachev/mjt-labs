...
public class SpotifyExplorer {
    ...
    public SpotifyExplorer(Reader dataInput) {
        try (BufferedReader br = new BufferedReader(dataInput)) {
            tracks = br.lines().skip(1).map(SpotifyTrack::of).collect(Collectors.toList());
        } catch (IOException e) {
    ...
    public int getArtistActiveYears(String artist) {
        List<Integer> artistSongYears =
                tracks.stream()
                .filter(t -> t.artists().contains(artist))
                .map(SpotifyTrack::year)
                .collect(Collectors.toList());

        int size = artistSongYears.size();
        if (size == 0) {
            return 0;
        }

        artistSongYears.sort(Comparator.naturalOrder());

        return artistSongYears.get(size - 1) - artistSongYears.get(0) + 1;
    }
    ...
    public List<SpotifyTrack> getTopNHighestValenceTracksFromThe80s(int n) {
        if (n < 0) {
            throw new IllegalArgumentException("N shouldn't be a negative number.");
        }
        return tracks.stream()
     ...
     *
     * @param minutes minimum minutes
     * @param year the year, before which the tracks should have been released
    ...
    public long getNumberOfLongerTracksBeforeYear(int minutes, int year) {
        if (minutes < 0 || year < 0) {
            throw new IllegalArgumentException("The track duration and release year should be positive numbers.");
        }
        return tracks.stream()
                .filter(t -> t.year() < year)
                .filter(t -> t.duration() > minutes * 60000)
                .count();
    }
    ...
    public Optional<SpotifyTrack> getTheLoudestTrackInYear(int year) {
        if (year < 0) {
            throw new IllegalArgumentException("The release year should be a positive number.");
        }
        return tracks.stream()
                .filter(t -> t.year() == year)
